...
#### Zone - Pitch Percent
zone_pitch_perc <- function(Batter, input) {
  ...
  df_pitches <- df %>%
    ...
    select(Batter_Last, PlayResult, AutoPitchType, Angle, ExitSpeed, RelSpeed, ynhit, pitch_zone) %>%
    group_by(pitch_zone) %>%
    summarise(
      Pitches = n(),
  ...
  ggplot() +
    ...
    geom_polygon(home_plate, mapping = aes(x, z), fill = "#ededed", color = "lightgrey") +
    coord_equal(xlim = c(-20, 20), ylim = c(0, 54)) +
    scale_fill_distiller(palette = "RdBu", limits = c(0, max(df_pitch_polys$Freq_PitchPercent) + 5),
                         breaks = c(0, max(df_pitch_polys$Freq_PitchPercent) + 5, 8),
    ...
    theme_minimal() +
    labs(
      subtitle = paste("Dates: ", min(input$Date), "to", max(input$Date))
    ) +
    TH +
    theme(
...
#### Zone - Pitch Percent
zone_total_pitches <- function(Pitcher, input) {
...
#### Zone - Exit Velocity
zone_exit_velo <- function(Batter, input) {
...
#### Zone - Launch Angle
zone_launch_angle <- function(Batter, input) {
...
#### Zone - Hit Percentage
zone_hit_perc <- function(Batter, input) {
...
zone_swing_perc <- function(Batter, input) {
  ...
  df_pitches <- df %>%
    filter(Batter_Last == Batter) %>%
    select(Batter_Last, PlayResult, AutoPitchType, Angle, ExitSpeed, RelSpeed, swing, pitch_zone) %>%
    group_by(pitch_zone) %>%
    summarise(
...
save(mutate_data, 
     ...
     zone_total_pitches, zone_pitch_perc, file = "data/functions.rda")
